...
public class TransformationRectangleFrame {
    private static double[][] nonSquareScreenMatrix = new double[4][4];

    public static Polygon toRectangleFrame(Polygon vertices){
        double[] z_value = TransformationPerspectiveMatrix.getZ_value();
        nonSquareScreenMatrix = SimpInterpreter.get_non_square_matrix();
        ...
        for(int index = 0; index < vertices.length(); index++){
            ...
            for(int i = 0; i < 4; i++) {
                double lineTotal = 0;
                for(int j = 0; j < 4; j++) {
                    lineTotal += nonSquareScreenMatrix[i][j] * current_point[j];
                }
                temp[i] = lineTotal;
            }
            current_point = temp;
